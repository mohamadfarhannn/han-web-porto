...
const baseClasses =
  "border-4 border-black font-black uppercase transition-all flex items-center justify-center gap-2 text-center text-black no-underline hover:translate-y-0.5 hover:translate-x-0.5 duration-200 disabled:opacity-70 disabled:cursor-not-allowed";

const computedClasses = computed(() => {
  return [baseClasses, bgMap[props.color] || bgMap.white, sizeClasses.value, props.block ? "w-full" : "inline-flex w-full sm:w-auto"];
});
</script>
...
<template>
  ...
  <button v-else :type="type" :disabled="disabled" :class="computedClasses">
    <slot />
  </button>
</template>
